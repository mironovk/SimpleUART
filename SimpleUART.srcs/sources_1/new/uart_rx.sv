module uart_rx (
  ...
  localparam logic speed = 115200;            // Частота передачи bit/s
  // localparam logic ticks_per_bit = 434;       //  Для скорости передачи 115200 bit/s
  localparam logic ticks_per_bit = clk_mhz * 1000 * 1000 / speed; //  Для скорости передачи 115200 bit/s

  localparam w_cnt = $clog2 (ticks_per_bit);  //  Вычисление требуемой разрядности счетчика

  logic [w_cnt:0] cnt = '0;                   //  Счетчик для обеспечения заданной частоты 115200
  logic [3:0] bit_num = 4'hF;
